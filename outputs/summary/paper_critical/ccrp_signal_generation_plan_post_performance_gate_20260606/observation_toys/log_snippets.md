# Toys Observation/Motivation Evidence Log Snippets
...
- 2026-06-11: ran `scripts/analysis/main_build_uncertainty_observation_study.py` on `pony-rec-gpu` for `toys` with `ccrp_uncertainty`, mean event aggregation, 5 bins, 10,000 events, 101 candidates per event, C-CRP plus LLMEmb/ProEx/RLMRec representative official baselines, and `min_join_rate=0.999`.
