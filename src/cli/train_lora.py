...
def _format_sft_rows(rows: list[dict[str, Any]]) -> list[dict[str, str]]:
    out = []
    for row in rows:
        prompt = (
        ...
        out.append({"prompt": prompt, "response": target})
    return out
...
def main() -> None:
    ...
    train_sft = _format_sft_rows(train_rows)
    valid_sft = _format_sft_rows(valid_rows)
    ...
    train_ds = Dataset.from_list(train_sft).map(tokenize, batched=True, remove_columns=["prompt", "response"])
    valid_ds = Dataset.from_list(valid_sft).map(tokenize, batched=True, remove_columns=["prompt", "response"])
