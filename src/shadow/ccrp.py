    ...
    *,
    ...
    raw_probability = clamp01(
        record.get(
            ...
            record.get("shadow_primary_score", record.get("confidence", 0.0)),
        )
    )
    calibrated_probability = clamp01(
        record.get(
            "calibrated_relevance_probability",
            record.get("calibrated_confidence", raw_probability),
        ),
    ...
    evidence_uncertainty = 1.0 - clamp01(evidence_support - counterevidence)
    uncertainty = clamp01(
        weights.boundary * boundary_uncertainty
        + weights.calibration_gap * calibration_gap
        + weights.evidence * evidence_uncertainty
    )
    if ablation == "without_risk_penalty":
        risk_adjusted_score = calibrated_probability
    else:
        risk_adjusted_score = calibrated_probability * ((1.0 - uncertainty) ** max(0.0, float(eta)))
...
def apply_ccrp_scores(
    ...
    *,
    ...
    out = df.copy()
    rows = [
        compute_ccrp_record(record, weights=weights, eta=eta, ablation=ablation)
