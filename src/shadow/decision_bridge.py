...
class SignalLookup:
    by_event_item: dict[tuple[str, str, str], dict[str, Any]]
    by_user_item: dict[tuple[str, str], dict[str, Any]]
...
def _event_id(record: dict[str, Any], *, fallback_index: int | None = None) -> str:
    ...
    if user_id and timestamp is not None and not _is_nan(timestamp):
        return f"{user_id}::{timestamp}"

    positive_item_id = str(record.get("positive_item_id") or record.get("target_item_id") or "").strip()
    if user_id and positive_item_id:
        return f"{user_id}::{positive_item_id}"
...
def build_signal_lookup(
    ...
    *,
    ...
    """Index calibrated winner-signal rows by event/item and user/item.

    Event-aware matches are preferred when `source_event_id` is available. The
    user/item lookup is kept as a fallback for older pointwise artifacts that do
    not store event ids.
    """
    ...
    for record in signal_records:
        ...
        if event_id:
            by_event_item[(event_id, user_id, item_id)] = entry
        by_user_item[(user_id, item_id)] = entry

    fallback_uncertainty = float(mean(uncertainties)) if uncertainties else 1.0
    return SignalLookup(
        by_event_item=by_event_item,
        by_user_item=by_user_item,
...
def build_shadow_v6_decision(
    *,
    ...
    """Convert one winner-signal row and one anchor score into a v6 target."""
    ...
    if not matched_signal:
        decision = {
            ...
            "pair_weight": 0.0,
            "reason": "Missing winner-signal row; the anchor ranking is retained.",
        }
    else:
        signal_reliability = 1.0 - signal_uncertainty
        conflict_discount = 1.0 - float(anchor_conflict_penalty) * anchor_disagreement
        raw_gate = _clamp01(signal_reliability * conflict_discount)
        ...
        pair_type = (
            "shadow_preferred_over_anchor"
            if not fallback_flag and signal_score >= anchor_score
            else "anchor_preferred_over_shadow"
        )
        weight_basis = max(signal_uncertainty, 1.0 - raw_gate) if fallback_flag else signal_reliability * correction_gate
    ...
    decision.update(compute_shadow_scores(decision, variant="shadow_v6"))
    return decision
...
def build_shadow_v6_bridge_rows(
    ...
    *,
    ...
    for record_index, record in enumerate(ranking_records):
        event_id = _event_id(record, fallback_index=record_index)
        user_id = str(record.get(user_col, "")).strip()
        ...
        for item_id in candidate_ids:
            event_key = (event_id, user_id, item_id)
            user_key = (user_id, item_id)
            entry = signal_lookup.by_event_item.get(event_key) or signal_lookup.by_user_item.get(user_key)
            if entry is not None:
                signal_entries[item_id] = entry
                matched_uncertainties.append(float(entry["signal_uncertainty"]))

        event_fallback_uncertainty = float(mean(matched_uncertainties)) if matched_uncertainties else signal_lookup.fallback_uncertainty
        ...
        for idx, item_id in enumerate(candidate_ids):
            ...
            signal_entry = signal_entries.get(item_id)
...
def build_shadow_v6_decision_predictions(
    ...
    *,
    ...
    original_lookup = {
        _event_id(record, fallback_index=idx): record
