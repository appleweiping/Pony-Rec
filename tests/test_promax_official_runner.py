...
PINNED_PROREC_COMMIT = "48a3a3d5075c1112c793b9bfc78de9a8b000ffe2"
...
def _args(tmp_path: Path, *, task_dir: Path, repo_dir: Path, dry_run: bool = False) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        valid_task_dir="",
        output_scores_path=str(output_dir / "promax_official_scores.csv"),
        ...
        promax_adapter_exp_name="",
        promax_adapter_dir="",
        promax_epochs=3000,
        promax_train_batch_size=4096,
        promax_lr=0.001,
        promax_weight_decay=0.0,
        promax_embedding_size=32,
        promax_layer_num=3,
        promax_reg_weight=1.0e-6,
        promax_sdr_weight=1.5,
        promax_s2dr_weight=0.05,
        promax_keep_rate=1.0,
        promax_rag_topk=20,
        promax_rag_block_users=128,
        promax_log_every=10,
    )


def _config() -> tuple[dict, dict, dict]:
    return resolve_method_config("configs/official_external_baselines.yaml", "promax")


def test_promax_official_runner_marks_completed_only_after_exact_scores(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.baselines.official_runner.promax.git_commit", lambda _repo: PINNED_PROREC_COMMIT)
    ...
    def fake_train(train_args: argparse.Namespace) -> dict[str, object]:
        ...
        _write_csv(
            ...
            [
                {"source_event_id": row["source_event_id"], "user_id": row["user_id"], "item_id": row["item_id"], "score": 3.0 - idx}
        ...
        return {
            ...
            "official_data_dir": str(tmp_path / "promax_data"),
            "profile_bridge_manifest_path": str(tmp_path / "profile_manifest.json"),
            "qwen3_profile_embedding_path": str(tmp_path / "pca64_itm_emb_np.pkl"),
            "qwen3_profile_embedding_source": "pca64_qwen3_item_embedding",
            "qwen3_profile_embedding_dim": 64,
            "rag_topk": 20,
            "latent_interaction_edges": 40,
            "trainable_params": 12,
        }

    monkeypatch.setattr("src.baselines.official_runner.promax.generate_sentence_embeddings", fake_embeddings)
    monkeypatch.setattr("src.baselines.official_runner.promax.train_and_score", fake_train)

    provenance = run_promax_official(args=args, cfg=cfg, method_cfg=method_cfg, contract=contract)
    ...
    assert provenance["runner_support_level"] == "official_promax_qwen3base_profile"
    assert provenance["official_training_config"]["sdr_weight"] == 1.5
    assert provenance["official_training_config"]["s2dr_weight"] == 0.05
    assert provenance["promax_latent_interaction_edges"] == 40
    assert provenance["same_candidate_score_audit"]["audit_ok"] is True


def test_promax_official_runner_blocks_hash_embeddings(tmp_path, monkeypatch):
    task_dir = _make_task(tmp_path)
    repo_dir = _make_repo(tmp_path)
    (tmp_path / "Qwen3-8B").mkdir()
    ...
    assert "promax_official_requires_real_text_embeddings_not_deterministic_hash" in provenance["blockers"]
