...
PINNED_LLMESR_COMMIT = "e5dc388c12509c88c65536ecd8d231325993d4ef"
...
def _args(tmp_path: Path, *, task_dir: Path, repo_dir: Path, dry_run: bool = False) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        valid_task_dir="",
        output_scores_path=str(output_dir / "llmesr_official_scores.csv"),
        ...
        llmesr_dataset_alias="",
        llmesr_adapter_exp_name="",
        llmesr_adapter_dir="",
        llmesr_epochs=200,
        llmesr_train_batch_size=128,
        llmesr_lr=0.001,
        llmesr_l2=0.0,
        llmesr_max_len=200,
        llmesr_hidden_size=64,
        llmesr_trm_num=2,
        llmesr_num_heads=1,
        llmesr_dropout_rate=0.5,
        llmesr_train_neg=1,
        llmesr_sim_user_num=10,
        llmesr_alpha=0.1,
        llmesr_beta=0.1,
        llmesr_user_sim_func="kd",
        llmesr_item_reg=False,
        llmesr_freeze=True,
        llmesr_use_cross_att=True,
        llmesr_log_every=5,
    )


def _config() -> tuple[dict, dict, dict]:
    return resolve_method_config("configs/official_external_baselines.yaml", "llmesr")


def test_llmesr_official_runner_marks_completed_only_after_exact_scores(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.baselines.official_runner.llmesr.git_commit", lambda _repo: PINNED_LLMESR_COMMIT)
    ...
    def fake_train(train_args: argparse.Namespace) -> dict[str, object]:
        ...
        return {"status": "completed", "checkpoint_path": train_args.checkpoint_path, "trainable_params": 12}

    monkeypatch.setattr("src.baselines.official_runner.llmesr.generate_sentence_embeddings", fake_embeddings)
    monkeypatch.setattr("src.baselines.official_runner.llmesr.train_and_score", fake_train)

    provenance = run_llmesr_official(args=args, cfg=cfg, method_cfg=method_cfg, contract=contract)
    ...
    assert provenance["runner_support_level"] == "official_llmesr_qwen3base_sasrec"
    assert provenance["official_training_config"]["num_train_epochs"] == 200
    assert provenance["same_candidate_score_audit"]["audit_ok"] is True


def test_llmesr_official_runner_blocks_without_real_run(tmp_path, monkeypatch):
    task_dir = _make_task(tmp_path)
    repo_dir = _make_repo(tmp_path)
    (tmp_path / "Qwen3-8B").mkdir()
    ...
    assert "llmesr_official_requires_real_text_embeddings_not_deterministic_hash" in provenance["blockers"]
