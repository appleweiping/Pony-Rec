...
PINNED_ELMREC_COMMIT = "b28c4f786d89fb8473ab358e12a882b30259f627"
...
def _args(tmp_path: Path, *, task_dir: Path, repo_dir: Path, dry_run: bool = False) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        valid_task_dir="",
        output_scores_path=str(output_dir / "elmrec_official_scores.csv"),
        ...
        elmrec_adapter_exp_name="",
        elmrec_adapter_dir="",
        elmrec_epochs=100,
        elmrec_train_batch_size=64,
        elmrec_lr=0.001,
        elmrec_alpha=6.0,
        elmrec_sigma=6.0,
        elmrec_graph_layers=4,
        elmrec_embedding_size=128,
        elmrec_reg_weight=1.0e-5,
        elmrec_log_every=5,
    )


def _config() -> tuple[dict, dict, dict]:
    return resolve_method_config("configs/official_external_baselines.yaml", "elmrec")


def test_elmrec_official_runner_marks_completed_only_after_exact_scores(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.baselines.official_runner.elmrec.git_commit", lambda _repo: PINNED_ELMREC_COMMIT)
    ...
    def fake_train(train_args: argparse.Namespace) -> dict[str, object]:
        ...
        return {
            ...
            "official_data_dir": str(tmp_path / "elmrec_data"),
            "trainable_params": 12,
        }

    monkeypatch.setattr("src.baselines.official_runner.elmrec.generate_sentence_embeddings", fake_embeddings)
    monkeypatch.setattr("src.baselines.official_runner.elmrec.train_and_score", fake_train)

    provenance = run_elmrec_official(args=args, cfg=cfg, method_cfg=method_cfg, contract=contract)
    ...
    assert provenance["runner_support_level"] == "official_elmrec_qwen3base_graph_bridge"
    assert provenance["official_training_config"]["alpha"] == 6.0
    assert provenance["official_training_config"]["graph_layers"] == 4
    assert provenance["same_candidate_score_audit"]["audit_ok"] is True


def test_elmrec_official_runner_blocks_hash_embeddings(tmp_path, monkeypatch):
    task_dir = _make_task(tmp_path)
    repo_dir = _make_repo(tmp_path)
    (tmp_path / "Qwen3-8B").mkdir()
    ...
    assert "elmrec_official_requires_real_text_embeddings_not_deterministic_hash" in provenance["blockers"]
