...
PINNED_RLMREC_COMMIT = "22413752246de3dee8ab0d509f7f7a8889080f95"
...
def _args(tmp_path: Path, *, task_dir: Path, repo_dir: Path, dry_run: bool = False) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        valid_task_dir="",
        output_scores_path=str(output_dir / "rlmrec_official_scores.csv"),
        ...
        rlmrec_adapter_exp_name="",
        rlmrec_adapter_dir="",
        rlmrec_epochs=3000,
        rlmrec_train_batch_size=4096,
        rlmrec_lr=0.001,
        rlmrec_weight_decay=0.0,
        rlmrec_embedding_size=32,
        rlmrec_layer_num=3,
        rlmrec_reg_weight=1.0e-5,
        rlmrec_cl_weight=1.0e-1,
        rlmrec_cl_temperature=0.2,
        rlmrec_kd_weight=1.0e-2,
        rlmrec_kd_temperature=0.2,
        rlmrec_eps=0.9,
        rlmrec_keep_rate=1.0,
        rlmrec_log_every=10,
    )


def _config() -> tuple[dict, dict, dict]:
    return resolve_method_config("configs/official_external_baselines.yaml", "rlmrec")


def test_rlmrec_official_runner_marks_completed_only_after_exact_scores(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.baselines.official_runner.rlmrec.git_commit", lambda _repo: PINNED_RLMREC_COMMIT)
    ...
    def fake_train(train_args: argparse.Namespace) -> dict[str, object]:
        ...
        return {
            ...
            "official_data_dir": str(tmp_path / "rlmrec_data"),
            "user_embedding_source": "train_history_mean_of_qwen_item_embeddings",
            "trainable_params": 12,
        }

    monkeypatch.setattr("src.baselines.official_runner.rlmrec.generate_sentence_embeddings", fake_embeddings)
    monkeypatch.setattr("src.baselines.official_runner.rlmrec.train_and_score", fake_train)

    provenance = run_rlmrec_official(args=args, cfg=cfg, method_cfg=method_cfg, contract=contract)
    ...
    assert provenance["runner_support_level"] == "official_rlmrec_qwen3base_simgcl_plus"
    assert provenance["official_training_config"]["model_name"] == "simgcl_plus"
    assert provenance["same_candidate_score_audit"]["audit_ok"] is True


def test_rlmrec_official_runner_blocks_hash_embeddings(tmp_path, monkeypatch):
    task_dir = _make_task(tmp_path)
    repo_dir = _make_repo(tmp_path)
    (tmp_path / "Qwen3-8B").mkdir()
    ...
    assert "rlmrec_official_requires_real_text_embeddings_not_deterministic_hash" in provenance["blockers"]
