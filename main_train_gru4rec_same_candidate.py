...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a lightweight GRU4Rec baseline and score exact candidate rows."
    )
...
def _import_torch():
    try:
        ...
        return torch, nn, pack_padded_sequence, pad_packed_sequence, DataLoader, TensorDataset
    except Exception as exc:
        raise RuntimeError("main_train_gru4rec_same_candidate.py requires PyTorch.") from exc
...
def _make_training_tensors(
    ...
    *,
    ...
    for seq in train_sequences.values():
        ...
        inputs.append(src + [0] * pad_len)
        targets.append(tgt + [0] * pad_len)
        lengths.append(actual_len)
    if not inputs:
        raise ValueError("No trainable GRU4Rec sequences found; need at least one user sequence with length >= 2.")
    return (
...
def _build_model_class(torch: Any, nn: Any):
    class GRU4RecModel(nn.Module):
        ...
        def forward(self, input_ids: Any, lengths: Any, pack_padded_sequence: Any, pad_packed_sequence: Any) -> tuple[Any, Any]:
            ...
            return output, self._last_hidden(output, lengths)

        @staticmethod
        def _last_hidden(output: Any, lengths: Any) -> Any:
...
def _train_model(
    *,
    ...
    DataLoader: Any,
    TensorDataset: Any,
    pack_padded_sequence: Any,
    pad_packed_sequence: Any,
    device: Any,
) -> list[dict[str, Any]]:
    dataset = TensorDataset(train_inputs, train_targets, train_lengths)
    ...
    for epoch in range(1, args.epochs + 1):
        total_loss = 0.0
        steps = 0
        for batch_inputs, batch_targets, batch_lengths in loader:
            batch_inputs = batch_inputs.to(device)
            batch_targets = batch_targets.to(device)
            batch_lengths = batch_lengths.to(device)
            optimizer.zero_grad(set_to_none=True)
            output, _ = model(batch_inputs, batch_lengths, pack_padded_sequence, pad_packed_sequence)
            logits = output @ model.item_embedding.weight.t()
        ...
        if args.log_every > 0 and (epoch == 1 or epoch == args.epochs or epoch % args.log_every == 0):
            print(f"[gru4rec] epoch={epoch} train_loss={mean_loss:.6f}")
    return logs


def _context_tensor(seq: list[str], item_to_idx: dict[str, int], *, max_seq_len: int, torch: Any, device: Any) -> tuple[Any, Any]:
    encoded = [item_to_idx[item_id] for item_id in seq if item_id in item_to_idx][-max_seq_len:]
    lengths = len(encoded)
    pad_len = max_seq_len - lengths
    return (
        torch.tensor([encoded + [0] * pad_len], dtype=torch.long, device=device),
    ...
    with output_scores_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        with torch.no_grad():
            for event in candidate_events:
                ...
                input_ids, lengths = _context_tensor(seq, item_to_idx, max_seq_len=max_seq_len, torch=torch, device=device)
                output, user_repr = model(input_ids, lengths, pack_padded_sequence, pad_packed_sequence)
                ...
                if not torch.isfinite(score_tensor).all():
                    raise RuntimeError(
                        "GRU4Rec produced non-finite candidate scores. "
...
def main() -> None:
    args = parse_args()
    torch, nn, pack_padded_sequence, pad_packed_sequence, DataLoader, TensorDataset = _import_torch()
    ...
    output_scores_path = Path(args.output_scores_path).expanduser() if args.output_scores_path else task_dir / "gru4rec_scores.csv"
    summary_path = task_dir / "gru4rec_train_summary.csv"
    epoch_log_path = task_dir / "gru4rec_epoch_log.csv"
    metadata_path = task_dir / "gru4rec_model_metadata.json"
    checkpoint_path = task_dir / "gru4rec_model.pt"
    ...
    train_inputs, train_targets, train_lengths = _make_training_tensors(
        train_sequences,
        item_to_idx,
    ...
    GRU4RecModel = _build_model_class(torch, nn)
    model = GRU4RecModel(
    ...
    print(
        f"[gru4rec] users={len(train_sequences)} train_sequences={len(train_inputs)} "
    ...
    print(f"[gru4rec] Saved scores: {output_scores_path}")
    print(f"[gru4rec] Saved summary: {summary_path}")
    print(f"[gru4rec] score_coverage_rate={score_summary['score_coverage_rate']:.6f}")
