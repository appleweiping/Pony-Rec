...
Generated by `main_import_same_candidate_baseline_scores.py` for external
baselines such as SASRec, BERT4Rec, GRU4Rec, LightGCN, or selected paper
adapters.

Rows are eligible for the unified method matrix only when:

- `status_label=same_schema_external_baseline`
- `score_coverage_rate=1.0` or missing-score handling is explicitly justified
- the score file was produced from the exported same-candidate task package
