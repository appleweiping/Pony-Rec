...
| **C (current yaml)** | restore standard `listwise_ranking_json_lora` text; `max_new_tokens=512`; `stop_at_json_end=false`; ASIN-safe decode | *read `repair_summary.json` per split after inference fully finishes* (`outputs/pilots/care_lora_qwen3_8b_beauty_100u_c19_seed42_strictgen/`) | Intended to recover baseline-like strict validity while keeping a higher token cap; **do not trust partial mid-run files**. |
