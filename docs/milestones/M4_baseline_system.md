...
| official-code-level LLM-rec | pinned LLM2Rec, LLM-ESR, LLMEmb, RLMRec, IRLLRec, SETRec | LLM2Rec, LLM-ESR, LLMEmb, RLMRec, and IRLLRec completed/imported across declared four-domain protocol; SETRec run-stage support implemented pending server validation |
...
imported and summarized. LLM-ESR, LLMEmb, RLMRec, and IRLLRec have now followed
that path across the declared domains. SETRec is the next runner-enabled method,
pending server rows with `official_completed`, `blockers=[]`, and exact score
coverage.
