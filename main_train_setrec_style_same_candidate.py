...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a SETRec-style order-agnostic identifier baseline using "
...
def _import_torch():
    try:
        ...
        raise RuntimeError("main_train_setrec_style_same_candidate.py requires numpy and PyTorch.") from exc
...
def _build_model_class(torch: Any, nn: Any):
    class SETRecIdentifierModel(nn.Module):
        def __init__(
            self,
            *,
            ...
            self.identifier_embedding = nn.Embedding(pretrained_item_matrix.shape[0], hidden_size, padding_idx=0)
            nn.init.normal_(self.identifier_embedding.weight, std=0.02)
        ...
        def item_representations(self) -> Any:
            ids = torch.arange(self.item_embedding.num_embeddings, device=self.item_embedding.weight.device)
            semantic = self.semantic_adapter(self.item_embedding.weight)
            identifier = self.identifier_embedding(ids)
            reps = semantic + identifier
            reps = reps.clone()
            reps[0] = 0.0
            return reps

        def encode_set(self, input_ids: Any) -> Any:
            item_reps = self.item_representations()
            hidden = item_reps.index_select(0, input_ids.reshape(-1)).reshape(*input_ids.shape, -1)
            mask = input_ids.ne(0).unsqueeze(-1)
            hidden = self.dropout(hidden * mask)
            counts = mask.sum(dim=1).clamp(min=1)
            mean_pool = hidden.sum(dim=1) / counts
            max_pool = hidden.masked_fill(~mask, -1.0e9).max(dim=1).values
            max_pool = torch.where(torch.isfinite(max_pool), max_pool, torch.zeros_like(max_pool))
            sum_pool = hidden.sum(dim=1)
            return self.set_encoder(torch.cat([mean_pool, max_pool, sum_pool], dim=-1))

        def score_all_items(self, input_ids: Any) -> Any:
            user_rep = self.encode_set(input_ids)
            item_reps = self.item_representations()
            return user_rep @ item_reps.t()

    return SETRecIdentifierModel
    ...
    *,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.log_every > 0 and (epoch == 1 or epoch == args.epochs or epoch % args.log_every == 0):
            print(f"[setrec-style] epoch={epoch} train_loss={mean_loss:.6f}", flush=True)
    return logs
        ...
        with torch.no_grad():
            for event in candidate_events:
                ...
                if not torch.isfinite(score_tensor).all():
                    raise RuntimeError("SETRec-style model produced non-finite candidate scores.")
...
def main() -> None:
    ...
    output_scores_path = Path(args.output_scores_path).expanduser() if args.output_scores_path else task_dir / "setrec_style_scores.csv"
    ...
    SETRecIdentifierModel = _build_model_class(torch, nn)
    model = SETRecIdentifierModel(
    ...
    print(
        f"[setrec-style] users={len(train_sequences)} train_examples={len(train_inputs)} "
    ...
    summary = {
        ...
        "artifact_class": "setrec_style_same_candidate_score",
        ...
        **score_summary,
        "note": (
            "Scores are exact same-candidate logits from a local SETRec-style "
            "order-agnostic identifier adapter with Qwen item embeddings. Label "
            "as SETRec-style unless the official SETRec pipeline is reproduced."
        ),
    }
