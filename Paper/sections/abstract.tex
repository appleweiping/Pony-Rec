...
behind the stronger baselines on 22 of 56 pairs. To show the advantage is not an
artifact of the unified Qwen3-8B backbone, we re-run the identical pipeline with a
second, different-family LLM (Llama-3.1-8B-Instruct) on the four new winning
domains: the rank-first NDCG@10 result replicates in all four ($+11.0\%$ to
$+21.4\%$ over the strongest baseline), at a $12$--$17\%$ lower absolute level
that tracks backbone quality.
