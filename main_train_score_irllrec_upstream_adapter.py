...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train pinned upstream IRLLRec LightGCN_int on a same-candidate "
            "adapter package and emit exact candidate scores."
        )
    )
    parser.add_argument("--adapter_dir", required=True)
    parser.add_argument("--irllrec_repo_dir", required=True)
    parser.add_argument("--output_scores_path", default=None)
    parser.add_argument("--checkpoint_path", default=None)
    parser.add_argument("--model_name", default="lightgcn_int")
    ...
    parser.add_argument("--kd_int_weight", type=float, default=2.0e-2)
...
def _import_runtime() -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    try:
        ...
        raise RuntimeError("The IRLLRec upstream wrapper requires numpy, scipy, and PyTorch.") from exc
...
def _candidate_groups(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        groups[key]["candidate_rows"].append(
            {"candidate_index": candidate_index, "item_id": item_id, "irllrec_item_idx": item_idx - 1}
        )
...
def _load_item_embeddings(adapter_dir: Path, np: Any) -> tuple[Any, Path]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"IRLLRec requires Qwen item embeddings at {path}")
...
def _load_item_intent_embeddings(adapter_dir: Path, np: Any) -> tuple[Any, Path]:
    path = adapter_dir / "llm_esr" / "handled" / "pca64_itm_emb_np.pkl"
    if not path.exists():
        raise FileNotFoundError(f"IRLLRec requires Qwen-derived item intent embeddings at {path}")
    with path.open("rb") as fh:
        matrix = pickle.load(fh)
    matrix = np.asarray(matrix, dtype=np.float32)
    if matrix.ndim != 2:
        raise ValueError(f"Expected 2D item intent embedding matrix, got shape={matrix.shape}")
    return matrix, path
...
def _build_user_intent_embeddings(*, sequences: dict[int, list[int]], item_intent_embeddings: Any, user_count: int, np: Any) -> tuple[Any, dict[str, Any]]:
    centered = item_intent_embeddings - item_intent_embeddings.mean(axis=0, keepdims=True)
    fallback = centered.mean(axis=0) if centered.shape[0] else np.zeros((centered.shape[1],), dtype=np.float32)
    user_intents = np.zeros((user_count, centered.shape[1]), dtype=np.float32)
    missing = 0
    for user_idx in range(user_count):
        item_indices = [idx for idx in sequences.get(user_idx, []) if 0 <= idx < centered.shape[0]]
        if item_indices:
            user_intents[user_idx] = centered[item_indices].mean(axis=0)
        else:
            user_intents[user_idx] = fallback
            missing += 1
    return user_intents, {
        "user_intent_embedding_source": "train_history_mean_of_centered_qwen_pca64_item_embeddings",
        "missing_intent_history_users": missing,
        "user_intent_embedding_dim": int(user_intents.shape[1]),
    }
...
def _make_sparse_mats(
    *,
    ...
    if not pairs:
        raise ValueError("No trainable IRLLRec user-item edges found.")
    data = [1.0] * len(rows)
...
def _write_official_data_artifacts(
    ...
    *,
    ...
    item_intent_embedding_source_path: Path,
) -> Path:
    out_dir = adapter_dir / "irllrec" / "handled"
    ...
    manifest = {
        ...
        "user_intent_embedding_source": "train_history_mean_of_centered_qwen_pca64_item_embeddings",
        "user_intent_embedding_shape": list(user_intent_emb.shape),
        "item_intent_embedding_source_path": str(item_intent_embedding_source_path),
        "item_intent_embedding_shape": list(item_intent_emb.shape),
    }
...
def _official_configs(
    ...
    *,
    ...
    hyper = {
        ...
        "kd_int_weight": args.kd_int_weight,
        "kd_int_temperature": args.kd_int_temperature,
        "kd_int_weight_2": args.kd_int_weight_2,
        "kd_int_weight_3": args.kd_int_weight_3,
    }
    return {
        "device": str(device),
        "usrprf_embeds": user_emb,
        "itmprf_embeds": item_emb,
        "usrint_embeds": user_intent_emb,
        "itmint_embeds": item_intent_emb,
...
def _checkpoint_config_summary(
    ...
    *,
    ...
    for key, value in configs.items():
        if key in {"usrprf_embeds", "itmprf_embeds", "usrint_embeds", "itmint_embeds"}:
            summary[key] = {
                "externalized": True,
    ...
    summary["itmint_embeds"]["source_path"] = str(item_intent_embedding_source_path)
    summary["usrint_embeds"]["source"] = "derived_from_train_history_qwen_pca64_item_intents"
    return summary
    ...
    *,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.log_every > 0 and (epoch == 1 or epoch == args.epochs or epoch % args.log_every == 0):
            print(f"[irllrec-official] epoch={epoch} train_loss={row['train_loss']:.6f}", flush=True)
    return logs
...
def _score_candidates(
    *,
    ...
    with torch.no_grad():
        user_embeds, item_embeds, _ = model.forward(model.adj, 1.0)
        for event in candidate_events:
            user_idx = int(event["irllrec_user_idx"])
            candidate_indices = [int(row["irllrec_item_idx"]) for row in event["candidate_rows"]]
            ...
            if not torch.isfinite(scores).all():
                raise RuntimeError(f"Non-finite IRLLRec score for source_event_id={event['source_event_id']}")
...
def train_and_score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    repo_dir = Path(args.irllrec_repo_dir).expanduser().resolve()
    ...
    output_scores_path = Path(args.output_scores_path).expanduser().resolve() if args.output_scores_path else adapter_dir / "irllrec_official_scores.csv"
    checkpoint_path = Path(args.checkpoint_path).expanduser().resolve() if args.checkpoint_path else adapter_dir / "irllrec_official_model.pt"
    ...
    item_intent_emb, item_intent_embedding_source_path = _load_item_intent_embeddings(adapter_dir, np)
    if item_intent_emb.shape[0] != item_count:
        raise ValueError(f"item intent embeddings rows={item_intent_emb.shape[0]} but adapter metadata items={item_count}")
    ...
    user_intent_emb, user_intent_embedding_summary = _build_user_intent_embeddings(
        sequences=sequences,
        item_intent_embeddings=item_intent_emb,
    ...
    handled_dir = _write_official_data_artifacts(
        ...
        user_emb=user_emb,
        item_emb=item_emb,
        user_intent_emb=user_intent_emb,
        item_intent_emb=item_intent_emb,
        item_embedding_source_path=item_embedding_source_path,
        item_intent_embedding_source_path=item_intent_embedding_source_path,
    )
    device = _resolve_device(args.device, torch)
    if device.type != "cuda":
        raise RuntimeError("Pinned IRLLRec intent models call .cuda(); run this official adapter on a CUDA device.")
    ...
    summary = {
        ...
        **score_summary,
        "note": (
            "Scores use pinned IRLLRec LightGCN_int with official BPR, semantic alignment, "
            "and intent representation losses. Local code supplies same-candidate graph data, "
            "Qwen3 item embeddings, Qwen3-PCA64 intent artifacts, train-history-derived user "
            "semantic/intent embeddings, and exact candidate-score export."
        ),
    }
    _write_csv([summary], adapter_dir / "irllrec_upstream_score_summary.csv", list(summary.keys()))
    _write_csv(logs, adapter_dir / "irllrec_upstream_epoch_log.csv", ["epoch", "train_loss"])
    (adapter_dir / "irllrec_upstream_score_metadata.json").write_text(
